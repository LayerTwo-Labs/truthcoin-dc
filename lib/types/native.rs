...
pub enum NativeOperationV1 {
    ...
    ClaimEscrow { #[schema(value_type = Vec<u8>)] escrow_id: NativeId, #[schema(value_type = Vec<u8>)] preimage: NativeId },
    /// Anyone may relay a refund after expiry; its destination is immutable.
    RefundEscrow { #[schema(value_type = Vec<u8>)] escrow_id: NativeId },
}

impl NativeOperationV1 {
    pub fn actor(&self) -> Option<Address> {
        match self {
            Self::TransferShares { owner, .. } | Self::LockShares { owner, .. } => Some(*owner),
            _ => None,
        }
    }
    ...
    pub fn nonce(&self) -> Option<(Address, NativeId)> {
        match self {
            Self::BuyForIntent { intent, .. } => Some((intent.recipient, intent.nonce)),
            Self::TransferShares { owner, nonce, .. } | Self::LockShares { owner, nonce, .. } => Some((*owner, *nonce)),
            _ => None,
        }
    }
}

#[derive(BorshSerialize, Clone, Debug, Deserialize, Serialize, PartialEq, Eq, ToSchema)]
...
pub enum EscrowStatusV1 {
    Locked,
    Claimed { #[schema(value_type = Vec<u8>)] transaction_id: NativeId },
    Refunded { #[schema(value_type = Vec<u8>)] transaction_id: NativeId },
}
...
#[derive(BorshSerialize, Clone, Copy, Debug, Deserialize, Serialize, PartialEq, Eq, ToSchema)]
pub enum NativeEffectKindV1 { Bought, Transferred, Locked, Claimed, Refunded }
...
mod tests {
    ...
    fn intent_signature_binds_genesis_recipient_quantity_and_reference() {
        ...
        let intent=BuyIntentV1 {genesis_hash:[1;32].into(),recipient:crate::authorization::get_address(&vk),
            market_id:MarketId::new([2;6]),outcome_index:1,shares:100,nonce:[3;32],reference:[4;32],valid_from_parent:10,valid_before_parent:20};
        let auth=Authorization {verifying_key:vk,signature:crate::authorization::sign(&key,crate::authorization::Dst::NativeIntent,&intent.signing_bytes().unwrap())};
        assert!(intent.verify(&auth));
        let mut changed=intent.clone();changed.genesis_hash=[2;32].into();assert!(!changed.verify(&auth));
        let mut changed=intent.clone();changed.recipient=Address([0;20]);assert!(!changed.verify(&auth));
        let mut changed=intent.clone();changed.shares+=1;assert!(!changed.verify(&auth));
        let mut changed=intent.clone();changed.reference[0]^=1;assert!(!changed.verify(&auth));
        let mut changed=intent.clone();changed.valid_before_parent+=1;assert!(!changed.verify(&auth));
        let wrong_domain=Authorization {verifying_key:vk,signature:crate::authorization::sign(&key,crate::authorization::Dst::Transaction,&intent.signing_bytes().unwrap())};
