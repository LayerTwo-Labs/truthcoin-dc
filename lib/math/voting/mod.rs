...
pub struct SparseVoteMatrix {
    ...
    decision_indices: HashMap<DecisionId, usize>,
...
impl SparseVoteMatrix {
    pub fn new(
        ...
        let decision_indices: HashMap<_, _> = decisions
            .iter()
            .enumerate()
            .map(|(j, &decision_id)| (decision_id, j))
            .collect();
    ...
    pub fn get_decisions(&self) -> &[DecisionId] {
        &self.decisions
    }
}
...
impl VotingWeightVector {
    ...
    pub fn from_reputation_map(
        reputations: &BTreeMap<crate::types::Address, f64>,
    ) -> Self {
        let mut weight_vector = Self::new();
        for (&address, &rep) in reputations {
            if rep > 0.0 {
                weight_vector.set_reputation(address, rep);
            }
        }
        weight_vector
    }
...
pub struct CategoricalResult {
    /// `Some(i)` with `i < option_count` indicates a single real option won.
    /// `Some(option_count)` indicates the decision resolved inconclusive
    /// (either because "Inconclusive" won the plurality or because the top
    /// options tied). `None` only occurs when no votes were cast.
    pub winning_option: Option<u16>,
}

#[derive(Debug, Clone)]
pub struct DetailedConsensusResult {
    pub outcomes: HashMap<DecisionId, Option<f64>>,
    pub first_loading: Vec<f64>,
    pub certainty: f64,
    pub updated_reputations: BTreeMap<crate::types::Address, f64>,
    pub outliers: Vec<crate::types::Address>,
    /// Per-decision plurality result. See [`CategoricalResult::winning_option`]
    /// for the meaning of the inner `Option<u16>`.
    pub categorical_winners: HashMap<DecisionId, Option<u16>>,
}

pub fn calculate_consensus(
    vote_matrix: &SparseVoteMatrix,
    reputation_vector: &VotingWeightVector,
    scaled_decisions: &HashSet<DecisionId>,
    categorical_decisions: &HashMap<DecisionId, u16>,
) -> Result<DetailedConsensusResult, VotingMathError> {
    oracle::run_consensus(
        vote_matrix,
        reputation_vector,
        scaled_decisions,
