...
/// Result of a proportional allocation (signed, for reputation deltas).
#[derive(Debug, Clone)]
...
///
/// Used for reputation redistribution where participants can have positive
...
mod tests {
    ...
    #[test]
    fn test_u64_pathological_over_allocation() {
        let participants: Vec<(usize, f64)> =
            (0..100).map(|i| (i, 1.0)).collect();
        let pool = 50u64;
        let result = allocate_proportionally_u64(participants, pool).unwrap();
    ...
    #[test]
    fn test_u64_many_tiny_allocations_conserve() {
        let participants: Vec<(usize, f64)> =
            (0..1000).map(|i| (i, 1.0)).collect();
        let pool = 1_000_001u64;
        let result = allocate_proportionally_u64(participants, pool).unwrap();
        let sum: u64 = result.allocations.iter().map(|(_, a)| *a).sum();
        assert_eq!(sum, pool);
    }
